...
- `googlePlaceId` (optional string)
- `mapsPlaceUrl` (optional URL)
- `lastVerified` (optional ISO date string)
- `status` (`active` or `closed`, defaults to `active`)
...
Weekly listing refresh/discovery automation is defined in `.github/workflows/weekly-data-refresh.yml`.

- Requires `GOOGLE_PLACES_API_KEY` in repository secrets.
- Runs refresh, discovery, and validation scripts and opens a PR when content changes are detected.
- Local run: `npm run data:weekly` (set `GOOGLE_PLACES_API_KEY` in your shell).

See `docs/listing-automation.md` for details.
